...
struct SheetButtonView: View {
    ...
    var body: some View {
        sheetButton(text: text)
    }
    
   
    private func sheetButton(text:String)-> some View{
        layer1(shape: RoundedRectangle(cornerRadius: 15.w))
            .frame(width: 100.w, height: 50.w)
            .overlay{
                
                Button(role: nil) {
                    onTab()
                } label:{
                    layer2(shape: RoundedRectangle(cornerRadius: 10.w))
                        .frame(width: 90.w, height: 40.w)
                        .overlay{
                            Text(text)
                                .font(.custom("CascadiaCodePL-Regular", size: 14.w))
